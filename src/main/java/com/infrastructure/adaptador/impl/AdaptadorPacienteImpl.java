...
public class AdaptadorPacienteImpl implements AdaptadorPaciente {
  ...
  public List<PacienteDTO> buscarTodosP() {
    return pacienteRepository.findAll().stream().map(mapperPaciente::toDTOPaciente).toList();
  }
  ...
  public List<PacienteDTO> buscarPacientesXMedico(String nLicencia) {
    List<PacienteDTO> listaPacientesDTO = pacienteRepository.buscarPacientesXMedico(nLicencia)
        .stream().map(mapperPaciente::toDTOPaciente).toList();
